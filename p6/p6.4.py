...
# Coordinates of the points given in the challenge
first = (
...
background_path = "ex6p4.jpg"
background = Image.open(background_path)
draw = ImageDraw.Draw(background)


# Function to draw lines between points with random color and fill the shape
def draw_lines(coords, color):
    """
    Draws lines between the given coordinates with the specified color.
    ...
    for i in range(0, len(coords) - 2, 2):
        draw.line(coords[i:i + 4], fill=color, width=2)


def fill_shape(coords, color):
    """
    Fills the shape defined by the given coordinates with the specified color.
    ...
    :type color: tuple
    """
    draw.polygon([(coords[i], coords[i + 1]) for i in range(0, len(coords), 2)], fill=color)
...
def random_color():
    ...
    :return: A tuple representing a random color
    :rtype: tuple
    """
    return tuple(random.randint(0, 255) for _ in range(3))


# Draw and fill the shapes on the background image
first_color = random_color()
second_color = random_color()

fill_shape(first, first_color)
fill_shape(second, second_color)
draw_lines(first, "black")
draw_lines(second, "black")

# Save the image with the connected dots
background.save("connect_the_dots_with_background_filled.png")
background.show()
